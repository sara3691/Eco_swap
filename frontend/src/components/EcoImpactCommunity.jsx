...
const EcoImpactCommunity = ({ userStats }) => {
    ...
    return (
        <div className="page-bg-container">
            ...
            <div className="relative z-10 py-12 md:py-24 px-4 md:px-6 max-w-7xl mx-auto space-y-12 md:space-y-20">
                {/* Header section */}
                <div className="text-center space-y-4 md:space-y-6">
                    <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="inline-flex items-center gap-2 px-4 md:px-6 py-1.5 md:py-2 rounded-full bg-white/95 dark:bg-clay-800/90 backdrop-blur-xl shadow-2xl border border-white/50 dark:border-white/10 text-eco-700 dark:text-eco-400 text-[10px] md:text-sm font-black uppercase tracking-[2px] md:tracking-[4px]"
                    >
                        <Users className="w-3 h-3 md:w-4 md:h-4" /> Live Community Impact
                    </motion.div>
                    <h2 className="text-4xl md:text-8xl font-black italic uppercase tracking-tighter text-white leading-tight md:leading-none drop-shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
                        Healing the <br />
                        <span className="eco-text-gradient">World Together.</span>
                    </h2>
                    <p className="text-white/90 text-base md:text-xl font-bold max-w-2xl mx-auto italic drop-shadow-md px-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                    <ImpactCard
                        icon={<RecycleIcon className="w-6 h-6 md:w-8 md:h-8 text-eco-500" />}
                        label="Total Swaps"
                    ...
                    />
                    <ImpactCard
                        icon={<Wind className="w-6 h-6 md:w-8 md:h-8 text-blue-500" />}
                        label="CO2 Reduced"
                    ...
                    />
                    <ImpactCard
                        icon={<Trash2 className="w-6 h-6 md:w-8 md:h-8 text-orange-500" />}
                        label="Plastic Saved"
                    ...
                    />
                    <ImpactCard
                        icon={<Droplets className="w-6 h-6 md:w-8 md:h-8 text-cyan-500" />}
                        label="Water Saved"
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
                    ...
                    <div className="space-y-8 md:space-y-12">
                        {/* User Mini Stats */}
                        <div className="glass-morphism p-6 md:p-10 rounded-[32px] md:rounded-[48px] bg-white/60 dark:bg-black/40 backdrop-blur-2xl border-white/20 space-y-6 md:space-y-8">
                            <div className="flex items-center gap-4 md:gap-6">
                                <div className="text-4xl md:text-6xl">{userStats?.badge_icon || '🌱'}</div>
                                <div>
                                    <h4 className="text-[9px] md:text-[10px] font-black uppercase tracking-[2px] md:tracking-[4px] text-eco-600 dark:text-eco-400">Your Rank</h4>
                                    <p className="text-2xl md:text-3xl font-black italic uppercase text-clay-900 dark:text-white">{userStats?.badge_name || 'Seed'}</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-3 md:gap-4">
                                <div className="bg-white/40 dark:bg-white/5 p-4 md:p-5 rounded-2xl md:rounded-3xl">
                                    <p className="text-[9px] md:text-[10px] font-black uppercase text-clay-400">Total Points</p>
                                    <p className="text-xl md:text-2xl font-black text-clay-900 dark:text-white">{userStats?.points || 0}</p>
                                </div>
                                <div className="bg-white/40 dark:bg-white/5 p-4 md:p-5 rounded-2xl md:rounded-3xl">
                                    <p className="text-[9px] md:text-[10px] font-black uppercase text-clay-400">Streak</p>
                                    <p className="text-xl md:text-2xl font-black text-orange-500 flex items-center gap-2">
                                        <Flame className="w-4 h-4 md:w-5 md:h-5" /> {userStats?.eco_streak || 0}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 md:space-y-6">
                            <h3 className="text-xl md:text-2xl font-black italic uppercase tracking-tight flex items-center gap-3 px-2 text-white dark:text-white drop-shadow-md">
                                <Zap className="w-5 h-5 md:w-6 md:h-6 text-orange-400" /> Active Challenges
                            </h3>
                            <ChallengeCard
                    ...
                    <div className="lg:col-span-1 space-y-4 md:space-y-6">
                        <h3 className="text-xl md:text-2xl font-black italic uppercase tracking-tight flex items-center gap-3 px-2 text-white dark:text-white drop-shadow-md">
                            <MessageSquare className="w-5 h-5 md:w-6 md:h-6 text-eco-500" /> Live Activity
                        </h3>
                        <div className="space-y-4 max-h-[500px] md:max-h-[800px] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <div className="space-y-4 md:space-y-6">
                        <h3 className="text-xl md:text-2xl font-black italic uppercase tracking-tight flex items-center gap-3 px-2 text-white dark:text-white drop-shadow-md">
                            <Trophy className="w-5 h-5 md:w-6 md:h-6 text-yellow-400" /> Top Leaders
                        </h3>
                        <div className="glass-morphism rounded-[32px] md:rounded-[48px] overflow-hidden bg-white/60 dark:bg-black/40 backdrop-blur-2xl border-white/20 divide-y divide-clay-100 dark:divide-white/5">
